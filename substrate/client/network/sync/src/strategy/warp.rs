		...
		match verifier.verify(&response) {
			...
			Ok(VerificationResult::Complete(header, proofs)) => {
				...
				let incoming_blocks: Vec<_> = proofs
					...
					.collect();
				self.phase = Phase::TargetBlock { header, verified_finalized_target: true };
				self.actions.push(SyncingAction::ImportBlocks {
					origin: BlockOrigin::WarpSync,
		...
		SyncStatus {
			state: match &self.phase {
				...
				Phase::TargetBlock { header, .. } => {
					SyncState::Downloading { target: *header.number() }
				},
...
mod test {
	...
	fn no_warp_proof_request_in_another_phase() {
		...
		warp_sync.phase = Phase::TargetBlock {
			header: <Block as BlockT>::Header::new(
				1,
				Default::default(),
				Default::default(),
				Default::default(),
	...
	fn complete_warp_proof_advances_phase() {
		...
		assert!(
			matches!(warp_sync.phase, Phase::TargetBlock { header, .. } if header == target_header)
		);
	}
	...
	fn externally_set_target_block_is_requested() {
		...
		assert!(matches!(warp_sync.phase, Phase::TargetBlock { .. }));
