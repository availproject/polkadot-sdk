			...
			match result {
				...
				Err(BlockImportError::IncompleteHeader(peer_id)) =>
					if let Some(peer) = peer_id {
						...
						self.actions
							.push(ChainSyncAction::DropPeer(BadPeer(peer.into(), rep::INCOMPLETE_HEADER)));
						self.restart();
					},
				Err(BlockImportError::VerificationFailed(peer_id, e)) => {
					...
					if let Some(peer) = peer_id {
						self.actions
							.push(ChainSyncAction::DropPeer(BadPeer(peer.into(), rep::VERIFICATION_FAIL)));
					}
				...
				Err(BlockImportError::BadBlock(peer_id)) =>
					if let Some(peer) = peer_id {
						...
						self.actions.push(ChainSyncAction::DropPeer(BadPeer(peer.into(), rep::BAD_BLOCK)));
					},
