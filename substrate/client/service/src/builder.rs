	...
	TExPool: MaintainedTransactionPool<Block = TBl, Hash = <TBl as BlockT>::Hash> + 'static,
{
	...
	let rpc_id_provider = if is_light_client { None } else { config.rpc.id_provider.take() };
	...
	if !is_light_client {
		...
		listen_addrs = rpc_server_handle
			.listen_addrs()
			.into_iter()
			.map(|socket_addr| {
		...
		// The in-memory RPC module still uses this dedicated runtime.
		task_manager.keep_alive((config.base_path, rpc_runtime));
	}
	...
	Net: NetworkBackend<Block, <Block as BlockT>::Hash>,
{
	...
	if client.requires_full_sync() {
		match net_config.network_config.sync_mode {
			SyncMode::AvailLight | SyncMode::LightState { .. } => {
