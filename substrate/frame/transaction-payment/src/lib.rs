...
use frame_support::{
	dispatch::{
		DispatchClass, DispatchFeeModifier, DispatchInfo, DispatchResult, GetDispatchInfo, Pays,
		PostDispatchInfo,
	},
...
use sp_runtime::{
	traits::{
		CheckedDiv, Convert, DispatchInfoOf, Dispatchable, One, PostDispatchInfoOf,
	...
	fn compute_fee_raw(
		...
		if pays_fee == Pays::Yes {
			...
			if let Some(fee_divider) = fee_modifier.weight_fee_divider {
				adjusted_weight_fee = adjusted_weight_fee
					.checked_div(&fee_divider.into())
					.unwrap_or_else(|| adjusted_weight_fee);
			}
	...
	BalanceOf<T>: Send + Sync,
{
	...
	pub fn get_priority(
		...
		// Since Avail uses 18 decimals, tipping will most of the time overflow the maximum tip
		// value of u64. To prevent that, the tip is adjusted but it also mean that a tip less
		// than 0.000000000001 won't be accounted for since the division will return 0.
